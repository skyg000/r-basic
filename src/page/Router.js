...
function Router() {
    ...
    return (
        <>
            ...
            @ 모튤설치  ( npm install react-router-dom )<br/>
            @ import &#123;Link,BrowserRouter,Route,Routes&#125; from 'react-router-dom';<br/><br/>
            <h2> Link, useLocation </h2>
            @ 브라우저에 URL값을 통해 값 전달할 때<br/>
                ㄴ Link 컨포넌트에 state속성을 이용함 (state = '값')<br/><br/>
            @ 브라우저에서 stater값 받기<br/>
                ㄴ react-router-dom에 있는 useLocation 훅을 사용<br/>
                ㄴ const 변수명 = useLocation();<br/>

            <Link to="/"> HOME </Link>
            <button onClick={move}>CSS활용</button>

            <h2> useNavigate(이동, 값 전달), useLocation(전달값 확인) </h2>
